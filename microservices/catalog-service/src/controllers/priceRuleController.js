...
export const getAllPriceRules = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi lấy luật giá:', error);
...
export const getActivePriceRules = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi lấy rule đang active:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
...
export const createPriceRule = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi tạo luật giá:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
...
export const updatePriceRule = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi cập nhật luật giá:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
...
export const deletePriceRule = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi xóa luật giá:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
