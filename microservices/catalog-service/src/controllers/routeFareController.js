...
export const getAllRouteFares = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi lấy bảng giá:', error);
...
export const getRouteFareById = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi lấy mức giá:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
...
export const createRouteFare = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi tạo mức giá:', error);
...
export const updateRouteFare = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi cập nhật mức giá:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
...
export const deleteRouteFare = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi xóa mức giá:', error);
    return res.status(500).json({ message: 'Lỗi hệ thống.' });
  }
};
