...
export const getRouteStops = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi khi lấy danh sách điểm dừng:', error);
...
export const getRouteStopById = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi khi lấy điểm dừng:', error);
    return res.status(500).json({ message: 'Đã xảy ra lỗi hệ thống.' });
  }
};
...
export const createRouteStop = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi khi thêm điểm dừng:', error);
    return res.status(500).json({ message: 'Đã xảy ra lỗi hệ thống.' });
  }
};
...
export const updateRouteStop = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi khi cập nhật điểm dừng:', error);
    return res.status(500).json({ message: 'Đã xảy ra lỗi hệ thống.' });
  }
};
...
export const deleteRouteStop = async (req, res) => {
  try {
    ...
    console.error('[Catalog Service] Lỗi khi xóa điểm dừng:', error);
    return res.status(500).json({ message: 'Đã xảy ra lỗi hệ thống.' });
  }
};
